...
// .grok/skills/* is the platform's locally-installed skill package, gitignored
// and never present in a plain checkout (a fresh clone or CI) — only inside a
// workspace the platform itself provisioned. This checks that installation's
// own docs against this script, so it has nothing to check without it.
const skillDir = join(TEMPLATE_ROOT, ".grok/skills/og");
const skillInstalled = existsSync(skillDir);

test(
  "every hand-over the og skill prints is one this script accepts",
  {
    skip:
      !skillInstalled && "no .grok/skills/og in this checkout (platform-provisioned, not in git)",
  },
  () => {
